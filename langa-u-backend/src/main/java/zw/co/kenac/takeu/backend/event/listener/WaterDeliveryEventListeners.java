...
@Slf4j
public class WaterDeliveryEventListeners {
    private final WaterDeliveryRepository deliveryRepository;
    private final FirebaseWaterService firebaseService;
    private final JavaMailService javaMailService;

    @EventListener(WaterDeliveryCreatedEvent.class)
    private void processDeliveryCreatedEvent(WaterDeliveryCreatedEvent event) {
        // todo pushe notification to the dispatcher
        firebaseService.createDelivery(event.getDelivery());
    }


    @EventListener(WaterDeliveryCompletedEvent.class)
    private void processDeliveryCompletedEvent(WaterDeliveryCompletedEvent event) {
        javaMailService.sendDeliveryCompletionEmail(event.getDeliveryId());
        firebaseService.updateActiveDeliveriesStatuses(event.getDeliveryId(), DeliveryStatus.valueOf(event.getDelivery().getDeliveryStatus()));
    }
    @EventListener(WaterDriverAcceptedEvent.class)
    private void processDeliveryAcceptedEvent(WaterDriverAcceptedEvent event) {
        javaMailService.sendDeliveryCompletionOtp(event.getDelivery().getEntityId());// we we send otps
        firebaseService.createActiveDeliveries(event.getDelivery());// here we update the record and send push notifcication
        // todo put push notification from here
    }
    @EventListener(WaterDeliveryOnRouteEvent.class)
    private void processDeliveryOnRouteEvent(WaterDeliveryOnRouteEvent event) {
    ...
    private void processDeliveryCreatedEvent(DispatchSettingsChangedEvent event) {
        ...
        firebaseService.updateAutoDispatchStatusInOpenDeliveries(event.getStatus());
    }
...
//        WaterDelivery delivery = deliveryRepository.findById(event.getDeliveryId())
